<?php
    ...
    for ($x = 1; $x < count($alphabet); $x++) 
    {
        ...
        if( isset( $_REQUEST[$alphabet[$x]] ))
        {
            ...
            $letters_As_Array = mysqli_fetch_array($result2);
            
            
                /*$letters_As_String = implode("",$letters_As_Array);
                echo $letters_As_String;*/
                //$letters_As_Array['Letter']."<br/>";
                // print_r($letters_As_Array['Letter']);
                
                //$resultSetLetters[] = $letters_As_Array;
                //print_r($resultSet);
            ...
            $result = array_intersect($word_Split_Array, array_pop($letters_As_Array);
            
            for ($x = 0; $x <= count($word_Split_Array); $x++) 
            {
                if ($result[$x] == TRUE)
                {
